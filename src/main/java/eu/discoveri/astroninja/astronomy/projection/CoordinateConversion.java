...
import eu.discoveri.astroninja.astronomy.projection.J2kCoordinateConversion;
import eu.discoveri.astroninja.astronomy.utils.AstroConst;
import eu.discoveri.astroninja.exception.DegreesOutOfRangeException;
import eu.discoveri.astroninja.exception.HoursOutOfRangeException;
import eu.discoveri.astroninja.utils.MathUtils;
...
public class CoordinateConversion
{
    ...
    /**
     * Converts a vector in J2000.0 coordinates to TEME (true equator, mean
     * equinox) of Date If you have more than one vector to transform you might
     * want to do this manually to save calculating rotation matrix many times
     *
    ...
    /**
     * Coverts a vector in TEME (true equator, mean equinox) of Date coordinates
     * to J2000.0
     *
     * @param mjd modified julian date of the desired coordinate transformation
     * @param vecTEME
    ...
    /**
     * Converts up to two J2000 vectors to MOD vectors (second vector set can be
     * null)
     *
     * @param mjd modified julian date of the desired coordinate transformation
    ...
    /**
     * Converts up to two J2000 vectors to TOD vectors (second vector set can be
     * null) delta psi/eps correction to J2000 are set to 0, uses 40 terms of
     * nutation
     *
     * @param mjd modified julian date of the desired coordinate transformation
     * @param j2kVec1 typically position
    ...
    public static void j2000toTOD(double mjd, double[] j2kVec1, double[] todVec1, double[] j2kVec2, double[] todVec2)
    {
        ...
        // careful on pass by reference, must copy values back to orginal array not assign a new object to it 
        double[] temp;
    ...
    /**
     * General function to perform a equinox basis transformation for a vector
     * in Earth equatorial coordinates (e.g., from 2000.0 to 1950.0)
     *
     * @param mjdCurrentEquinox Current Modified Julian Date Equinox
     * @param currentVec Current vector in Earth equatorial coordinates using
     * mjdCurrentEquinox equinox date (any units of length)
    ...
    /**
     * Converts J2000.0 Coordinates to a new equinox date (such as J2000.0 to
     * Mean of Date, mjdNewEquinox = current date)
     *
    ...
    /**
     * Converts Equatorial Coordinates from any equinox to J2000.0 (such as Mean
     * of Date to J2000.0, mjdCurrentEquinox = current date) NOTE: See
     * J2kCoordinateConversion.java - for possibly better ways to do this
     *
    ...
    /**
     * Precession of equatorial coordinates, similar to PrecMatrix_Equ, except
     * inputs are Modified Julian Dates
     *
    ...
    public static double[][] NutMatrix(double Mjd_TT)
    {
        ...
        double[] deps_dpsi = NutAngles(Mjd_TT);
     ...
     *
     * Notes: The equation of the equinoxes dpsi*cos(eps) is the right ascension
     * of the mean equinox referred to the true equator and equinox and is equal
    ...
    {
        // Nutation in longitude and obliquity  
        double[] deps_dpsi = NutAngles(Mjd_TT);    // Nutation angles  
     ...
     * 
     * @param rightAngle, decimal hours (0.0 - 24.0)
     * @param declination, Decimal degrees (0.0 - 360.0)
     * @param dist, parsecs (ly to parsec, divide by 3.262)
     * @return 
     * @throws HoursOutOfRangeException
     * @throws DegreesOutOfRangeException
     */
    public static CartesianCoordinates3D convertSphericalToCartesian( double rightAngle, double declination, double dist )
            throws HoursOutOfRangeException, DegreesOutOfRangeException
    {
        if( rightAngle < 0.0 || rightAngle > 24.0 )
            throw new HoursOutOfRangeException("0.0 <= rightAngle <= 24.0");
        if( declination < 0.0 || declination > 360.0 )
            throw new DegreesOutOfRangeException("0.0 <= declination <= 360.0");
        
        double phi = Math.toRadians(rightAngle*15.0);
        double theta = Math.toRadians(declination);
        double rvect = dist * Math.cos(theta);
        
        return new CartesianCoordinates3D( rvect*Math.cos(phi), rvect*Math.sin(phi), dist*Math.sin(theta) );
    }
    
    /**
     * Convert an angle in degrees (such as azimuth) to compass points (N, NNE,
     * NE...etc)
     *
     * @param degrees (will be converted to [0,360) )
     * @return compass direction -- if you only want N, NE, E... then check for
     * a length of 3 and drop first char
     */
